...
interface OtherIncomeStatementDialogProps {
  open: boolean;
...
const STORAGE_KEY = 'biskit_sps_oi_creation_form';
...
export function OtherIncomeStatementDialog({
  ...
  const handleSubmit = async (): Promise<void> => {
    ...
    try {
      ...
      // Alert 노출
      alert(`${year}년 ${month}월 기타소득 간이지급명세서 생성이 완료되었습니다.`);
  ...
  const updateReportFileGeneratedAt = (
    ...
    if (stored) {
      ...
      localStorage.setItem('biskit_other_income', JSON.stringify(updated));
    }
  };
...
%%EOF`;
    ...
    link.href = url;
    link.download = `${year}년 ${month}월 기타소득 간이지급명세서.pdf`;
    link.click();
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>기타소득 간이지급명세서 생성</DialogTitle>
        </DialogHeader>
        ...
        <div className="mb-6 p-4 bg-gray-50 rounded">
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600">총 필요경비</span>
              <span className="text-sm font-semibold">
                {formatCurrency(summary.totalNecessaryExpenses)}
              </span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600">총 소득금액</span>
              <span className="text-sm font-semibold">
                {formatCurrency(summary.totalIncomeAmount)}
              </span>
            </div>
          </div>
        </div>
