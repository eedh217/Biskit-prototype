...
import { useCreateAllOtherIncome } from '../hooks/useOtherIncome';
import {
...
interface AllOtherIncomeAddDialogProps {
  open: boolean;
...
export function AllOtherIncomeAddDialog({
  open,
  onOpenChange,
}: AllOtherIncomeAddDialogProps): JSX.Element {
  const createMutation = useCreateAllOtherIncome();
  ...
  const handleClose = (): void => {
    if (isDirty) {
      const confirmed = window.confirm('기타소득 추가를 취소하시겠습니까?');
      if (confirmed) {
        reset();
        onOpenChange(false);
      }
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>기타소득 추가</DialogTitle>
        </DialogHeader>
