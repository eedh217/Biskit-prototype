...
interface AllOtherIncomeEditDialogProps {
  open: boolean;
...
const paymentYearOptions = Array.from(
  { length: currentYear - 2025 + 2 }, // 2026 ~ (현재연도 + 1)
  (_, i) => 2026 + i
);
const attributionYearOptions = Array.from(
  { length: currentYear - 2025 + 1 }, // 2026 ~ 현재연도
  (_, i) => 2026 + i
);
const monthOptions = Array.from({ length: 12 }, (_, i) => i + 1);

export function AllOtherIncomeEditDialog({
  open,
  onOpenChange,
  otherIncomeId,
}: AllOtherIncomeEditDialogProps): JSX.Element {
  const { data: otherIncome, isLoading } = useOtherIncome(otherIncomeId || '');
  const updateMutation = useUpdateAllOtherIncome();
  ...
  const paymentYear = watch('paymentYear');
  const paymentMonth = watch('paymentMonth');
  ...
  useEffect(() => {
    if (otherIncome) {
      ...
      reset({
        paymentYear: String(otherIncome.paymentYear),
        paymentMonth: String(otherIncome.paymentMonth),
        ...
        paymentAmount: formatNumber(otherIncome.paymentAmount),
        necessaryExpenses: formatNumber(otherIncome.necessaryExpenses),
      });
    }
  }, [otherIncome, reset]);
  ...
  const validateAttributionDate = (): void => {
    if (paymentYear && paymentMonth && attributionYear && attributionMonth) {
      const attrDate = new Date(
        Number(attributionYear),
        Number(attributionMonth) - 1
      );
      const payDate = new Date(Number(paymentYear), Number(paymentMonth) - 1);
  ...
  }, [paymentYear, paymentMonth, attributionYear, attributionMonth]);
  ...
  const handleDelete = async (): Promise<void> => {
    ...
    if (confirmed) {
      try {
        await deleteMutation.mutateAsync(otherIncomeId);
  ...
  const onSubmit = async (data: FormData): Promise<void> => {
    ...
    if (
      data.paymentYear &&
      data.paymentMonth &&
      data.attributionYear &&
      data.attributionMonth
    ) {
      const attrDate = new Date(
        Number(data.attributionYear),
        Number(data.attributionMonth) - 1
      );
      const payDate = new Date(
        Number(data.paymentYear),
        Number(data.paymentMonth) - 1
      );
    ...
    // 필요경비 검증 (지급액 0원 허용)
    if (paymentAmount === 0 && necessaryExpenses > 0) {
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <Label htmlFor="paymentYear">
                지급연도 <span className="text-red-500">*</span>
              </Label>
              <Controller
                name="paymentYear"
                control={control}
                render={({ field }) => (
                  <Select
                    ...
                    onValueChange={(value) => {
                      ...
                      if (value) clearErrors('paymentYear');
                    }}
                  >
                    <SelectTrigger>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <Select
                    ...
                    onValueChange={(value) => {
                      field.onChange(value);
                      if (value) clearErrors('paymentMonth');
                    }}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="선택" />
              ...
              <Controller
                ...
                render={({ field }) => (
                  ...
                  >
                    ...
                    <SelectContent>
                      {attributionYearOptions.map((year) => (
                        <SelectItem key={year} value={String(year)}>
                          {year}년
                        </SelectItem>
                      ))}
          ...
          <div>
            ...
            <Controller
              ...
              render={({ field }) => (
                ...
                >
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="ADVISORY" id="advisory-edit" />
                    <Label htmlFor="advisory-edit" className="cursor-pointer">
                      자문/고문
                    </Label>
                  </div>
                  <div className="flex items-center space-x-2 relative">
                    <RadioGroupItem
                      ...
                      id="other-service-edit"
                    />
                    <Label htmlFor="other-service-edit" className="cursor-pointer">
                      자문/고문 외 인적용역
                    </Label>
                    <button
                      type="button"
