import { useState, useEffect, useMemo } from 'react';
import { Search, Trash2, Plus } from 'lucide-react';
import { format } from 'date-fns';
import { PageHeader } from '@/shared/components/common/PageHeader';
import { Button } from '@/shared/components/ui/button';
import { Input } from '@/shared/components/ui/input';
import { Label } from '@/shared/components/ui/label';
import { Card, CardContent, CardHeader, CardTitle } from '@/shared/components/ui/card';
import { Checkbox } from '@/shared/components/ui/checkbox';
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
} from '@/shared/components/ui/dialog';
import {
  Table,
  TableBody,
  TableCell,
  TableHead,
  TableHeader,
  TableRow,
} from '@/shared/components/ui/table';
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from '@/shared/components/ui/select';
import { EmployeeCombobox } from '../components/EmployeeCombobox';
import { AddressSearchDialog } from '../components/AddressSearchDialog';
import { toast } from '@/shared/hooks/use-toast';
import {
  WorkplaceInfo,
  EmployeeTotalSalaryInfo,
  InsuranceTotalSalaryForm,
} from '../types/insurance';
import {
  saveWorkplaceInfo,
  loadWorkplaceInfo,
  saveTotalSalaryHistory,
  getTotalSalaryHistories,
} from '../services/insuranceService';
import type { Employee } from '../types/employee';

const TOTAL_SALARY_TEMP_STORAGE_KEY = 'biskit_insurance_total_salary_temp';

// 기본 직원 데이터 생성 함수
const createDefaultEmployee = (): EmployeeTotalSalaryInfo => {
  // 전년도를 기본 신고년도로 설정
  const currentYear = new Date().getFullYear();
  const defaultReportYear = (currentYear - 1).toString();

  return {
    employeeId: '',
    employeeNumber: '',
    name: '',
    residentNumber: '',
    reportYear: defaultReportYear,
    acquisitionDate: '',
    applyPension: true,
    applyHealthInsurance: true,
    applyEmploymentInsurance: true,
    applyWorkersCompensation: true,
    pensionReportType: '',
    pensionTotalSalary: 0,
    pensionWorkStartDate: '',
    pensionHasLeave: false,
    pensionLeaveDays: 0,
    healthTotalSalary: 0,
    healthWorkMonths: 0,
    healthInsurancePremiumTotal: 0,
    employmentTotalSalary: 0,
    workersCompTotalSalary: 0,
  };
};

export function InsuranceTotalSalary(): JSX.Element {
  const [isComposing, setIsComposing] = useState(false);
  const [addressDialogOpen, setAddressDialogOpen] = useState(false);

  // 사업장 정보
  const [workplace, setWorkplace] = useState<WorkplaceInfo>({
    managementNumber: '',
    name: '',
    unitName: '',
    branchName: '',
    postalCode: '',
    address: '',
    addressDetail: '',
    phoneNumber: '',
    faxNumber: '',
    email: '',
    mobilePhone: '',
  });

  // 직원 목록 (초기에 빈 직원 1명 추가)
  const [employees, setEmployees] = useState<EmployeeTotalSalaryInfo[]>([createDefaultEmployee()]);
  const [selectedEmployeeIds, setSelectedEmployeeIds] = useState<Set<string>>(new Set());

  // 국민연금 근무시작일 에러 상태 (index: error message)
  const [pensionWorkStartDateErrors, setPensionWorkStartDateErrors] = useState<Record<number, string>>({});

  // 국민연금 휴직일수 에러 상태 (index: error message)
  const [pensionLeaveDaysErrors, setPensionLeaveDaysErrors] = useState<Record<number, string>>({});

  // 임시저장 정보
  const [hasTempData, setHasTempData] = useState(false);
  const [tempSavedAt, setTempSavedAt] = useState<string>('');

  // 신고내역 조회 Dialog
  const [historyDialogOpen, setHistoryDialogOpen] = useState(false);
  const [historyStartDate, setHistoryStartDate] = useState('');
  const [historyEndDate, setHistoryEndDate] = useState('');
  const [historySearch, setHistorySearch] = useState('');
  const [histories, setHistories] = useState<any[]>([]);
  const [selectedPeriod, setSelectedPeriod] = useState<string>('1month');
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;

  // 신고 확인 Dialog
  const [confirmDialogOpen, setConfirmDialogOpen] = useState(false);
  const [faxNumber, setFaxNumber] = useState('');

  // 스크롤 위치 감지 (하단 버튼 영역 그림자 제어)
  const [isAtBottom, setIsAtBottom] = useState(false);

  // 초기 데이터 로드
  useEffect(() => {
    const savedWorkplace = loadWorkplaceInfo();
    if (savedWorkplace) {
      setWorkplace(savedWorkplace);
    }

    const tempDataStr = localStorage.getItem(TOTAL_SALARY_TEMP_STORAGE_KEY);
    if (tempDataStr) {
      try {
        const tempData = JSON.parse(tempDataStr);
        setHasTempData(true);
        setTempSavedAt(tempData.savedAt || '');
      } catch {
        // 파싱 실패 시 무시
      }
    }

    // 스크롤 이벤트 리스너
    const mainElement = document.querySelector('main');
    const handleScroll = (): void => {
      if (mainElement) {
        const { scrollTop, scrollHeight, clientHeight } = mainElement;
        setIsAtBottom(scrollTop + clientHeight >= scrollHeight - 10);
      }
    };

    if (mainElement) {
      mainElement.addEventListener('scroll', handleScroll);
      handleScroll(); // 초기 상태 확인
    }

    return () => {
      if (mainElement) {
        mainElement.removeEventListener('scroll', handleScroll);
      }
    };
  }, []);

  // 사업장 정보 변경
  const handleWorkplaceChange = (field: keyof WorkplaceInfo, value: string): void => {
    setWorkplace((prev) => ({ ...prev, [field]: value }));
  };

  // 주소 검색 완료
  const handleAddressComplete = (data: { zonecode: string; address: string }): void => {
    setWorkplace((prev) => ({
      ...prev,
      postalCode: data.zonecode,
      address: data.address,
    }));
    setAddressDialogOpen(false);
  };

  // 직원 정보 변경
  const handleEmployeeChange = (
    index: number,
    field: keyof EmployeeTotalSalaryInfo,
    value: string | number | boolean
  ): void => {
    setEmployees((prev) => {
      const updated = [...prev];
      const employee = { ...updated[index] } as EmployeeTotalSalaryInfo;
      (employee[field] as typeof value) = value;
      updated[index] = employee;
      return updated;
    });
  };

  // 주민등록번호/외국인등록번호 포맷팅 (000000-0000000)
  const handleResidentNumberChange = (index: number, value: string): void => {
    // 숫자만 추출
    const numbers = value.replace(/[^0-9]/g, '');

    // 13자리 제한
    const limited = numbers.slice(0, 13);

    // 6자리 이후 하이픈 자동 추가
    let formatted = limited;
    if (limited.length > 6) {
      formatted = `${limited.slice(0, 6)}-${limited.slice(6)}`;
    }

    handleEmployeeChange(index, 'residentNumber', formatted);
  };

  // 국민연금 근무시작일 검증 (MM-DD 형식)
  const validatePensionWorkStartDate = (value: string): boolean => {
    if (!value || value.length < 5) return false;

    const parts = value.split('-');
    if (parts.length !== 2) return false;

    const monthStr = parts[0];
    const dayStr = parts[1];

    if (!monthStr || !dayStr) return false;

    const month = parseInt(monthStr, 10);
    const day = parseInt(dayStr, 10);

    // 월 검증 (01-12)
    if (isNaN(month) || month < 1 || month > 12) return false;

    // 각 월별 최대 일수
    const daysInMonth = [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
    const maxDay = daysInMonth[month - 1];

    if (!maxDay) return false;

    // 일 검증
    if (isNaN(day) || day < 1 || day > maxDay) return false;

    return true;
  };

  // 국민연금 근무시작일 blur 핸들러
  const handlePensionWorkStartDateBlur = (index: number, value: string): void => {
    // 빈 값은 에러 제거
    if (!value) {
      setPensionWorkStartDateErrors((prev) => {
        const updated = { ...prev };
        delete updated[index];
        return updated;
      });
      return;
    }

    // 날짜 검증
    if (!validatePensionWorkStartDate(value)) {
      setPensionWorkStartDateErrors((prev) => ({
        ...prev,
        [index]: '유효한 날짜가 아닙니다.',
      }));
    } else {
      setPensionWorkStartDateErrors((prev) => {
        const updated = { ...prev };
        delete updated[index];
        return updated;
      });
    }
  };

  // 국민연금 휴직일수 blur 핸들러
  const handlePensionLeaveDaysBlur = (index: number, value: number): void => {
    // 365 초과 여부 검증
    if (value > 365) {
      setPensionLeaveDaysErrors((prev) => ({
        ...prev,
        [index]: '신고년도 기간 중 휴직일수를 입력해주세요.',
      }));
    } else {
      setPensionLeaveDaysErrors((prev) => {
        const updated = { ...prev };
        delete updated[index];
        return updated;
      });
    }
  };

  // 콤보박스에서 직원 선택 시 정보 자동 입력
  const handleSelectEmployeeFromCombobox = (index: number, employee: Employee | null): void => {
    if (!employee) return;

    // 전년도를 기본 신고년도로 설정
    const currentYear = new Date().getFullYear();
    const defaultReportYear = (currentYear - 1).toString();

    // 입사일 형식 변환: YYYYMMDD → YYYY-MM-DD
    let formattedJoinDate = '';
    if (employee.joinDate && employee.joinDate.length === 8) {
      const year = employee.joinDate.substring(0, 4);
      const month = employee.joinDate.substring(4, 6);
      const day = employee.joinDate.substring(6, 8);
      formattedJoinDate = `${year}-${month}-${day}`;
    }

    setEmployees((prev) => {
      const updated = [...prev];
      updated[index] = {
        ...updated[index],
        employeeId: employee.id,
        employeeNumber: employee.employeeNumber,
        name: employee.name,
        residentNumber: employee.residentRegistrationNumber || employee.foreignerRegistrationNumber || '',
        reportYear: defaultReportYear,
        acquisitionDate: formattedJoinDate, // 입사일을 자격취득일로 설정
      } as EmployeeTotalSalaryInfo;
      return updated;
    });
  };

  // 직원 삭제 (다중)
  const handleDeleteEmployees = (): void => {
    if (selectedEmployeeIds.size === 0) {
      toast({
        title: '삭제할 직원을 선택하세요.',
        variant: 'destructive',
      });
      return;
    }

    setEmployees((prev) =>
      prev.filter((_, index) => !selectedEmployeeIds.has(String(index)))
    );
    setSelectedEmployeeIds(new Set());

    // 에러 상태도 재정렬
    setPensionWorkStartDateErrors({});
    setPensionLeaveDaysErrors({});
  };

  // 직원 삭제 (단일)
  const handleDeleteSingleEmployee = (index: number): void => {
    setEmployees((prev) => prev.filter((_, i) => i !== index));
    // 선택 목록에서도 제거
    const newSelected = new Set(selectedEmployeeIds);
    newSelected.delete(String(index));
    setSelectedEmployeeIds(newSelected);

    // 에러 상태도 재정렬
    setPensionWorkStartDateErrors((prev) => {
      const updated: Record<number, string> = {};
      Object.entries(prev).forEach(([key, value]) => {
        const idx = parseInt(key, 10);
        if (idx < index) {
          updated[idx] = value;
        } else if (idx > index) {
          updated[idx - 1] = value;
        }
      });
      return updated;
    });

    setPensionLeaveDaysErrors((prev) => {
      const updated: Record<number, string> = {};
      Object.entries(prev).forEach(([key, value]) => {
        const idx = parseInt(key, 10);
        if (idx < index) {
          updated[idx] = value;
        } else if (idx > index) {
          updated[idx - 1] = value;
        }
      });
      return updated;
    });
  };

  // 임시 저장
  const handleTempSave = (): void => {
    const formData: InsuranceTotalSalaryForm = {
      workplace,
      employees,
      reportDate: format(new Date(), 'yyyy-MM-dd'),
    };

    const now = new Date().toISOString();
    const dataToSave = {
      ...formData,
      savedAt: now,
    };

    localStorage.setItem(TOTAL_SALARY_TEMP_STORAGE_KEY, JSON.stringify(dataToSave));
    setHasTempData(true);
    setTempSavedAt(now);

    toast({
      title: '임시 저장되었습니다.',
    });
  };

  // 임시 저장 불러오기
  const handleLoadTempData = (): void => {
    const tempDataStr = localStorage.getItem(TOTAL_SALARY_TEMP_STORAGE_KEY);
    if (!tempDataStr) {
      toast({
        title: '불러올 임시 저장 데이터가 없습니다.',
        variant: 'destructive',
      });
      return;
    }

    // 현재 작성 중인 데이터가 있는지 확인
    const hasCurrentData =
      workplace.managementNumber ||
      workplace.name ||
      workplace.postalCode ||
      workplace.address ||
      workplace.addressDetail ||
      employees.some(
        (emp) =>
          emp.name ||
          emp.residentNumber ||
          emp.reportYear
      );

    // 현재 데이터가 있으면 확인 메시지 표시
    if (hasCurrentData) {
      if (
        !window.confirm(
          '임시저장 데이터를 불러올 경우, 현재 입력한 데이터는 사라집니다. 불러오시겠습니까?'
        )
      ) {
        return;
      }
    }

    try {
      const tempData: InsuranceTotalSalaryForm = JSON.parse(tempDataStr);
      setWorkplace(tempData.workplace);
      setEmployees(tempData.employees);

      toast({
        title: '임시 저장 데이터를 불러왔습니다.',
      });
    } catch {
      toast({
        title: '임시 저장 데이터를 불러오는데 실패했습니다.',
        variant: 'destructive',
      });
    }
  };

  // 임시 저장 삭제
  const handleClearTempData = (): void => {
    localStorage.removeItem(TOTAL_SALARY_TEMP_STORAGE_KEY);
    setHasTempData(false);
    setTempSavedAt('');

    toast({
      title: '임시 저장 데이터를 삭제했습니다.',
    });
  };

  // 신고 확인 다이얼로그용 주소 추출
  // 서울: 구까지, 광역시: 구까지, 도 지역: 시까지 표시
  const getShortAddress = (fullAddress: string): string => {
    // 서울특별시 또는 서울인 경우 "서울 구명"으로 표시
    if (fullAddress.includes('서울특별시') || fullAddress.startsWith('서울 ')) {
      const match = fullAddress.match(/(서울특별시|서울)\s*(.*?구)/);
      return match ? `서울 ${match[2]}` : fullAddress.split(' ').slice(0, 2).join(' ');
    }

    // 광역시인 경우 "시명 구명"으로 표시
    if (fullAddress.includes('광역시')) {
      const match = fullAddress.match(/(.*?)광역시\s*(.*?구)/);
      return match ? `${match[1]} ${match[2]}` : fullAddress.split(' ').slice(0, 2).join(' ');
    }

    // 도 지역인 경우 "도명 시명"으로 표시
    const match = fullAddress.match(/(.*?도)\s*(.*?시)/);
    return match ? `${match[1]} ${match[2]}` : (fullAddress.split(' ')[0] ?? '');
  };

  // 필수값 검증
  const isFormValid = useMemo(() => {
    // 사업장 정보 필수값 검증
    if (!workplace.managementNumber || !workplace.name || !workplace.postalCode ||
        !workplace.address || !workplace.addressDetail || !workplace.phoneNumber) {
      return false;
    }

    // 직원이 없으면 false
    if (employees.length === 0) {
      return false;
    }

    // 직원 정보 필수값 검증
    for (const emp of employees) {
      // 기본 정보
      if (!emp.employeeId || !emp.name || !emp.residentNumber || !emp.reportYear) {
        return false;
      }

      // 국민연금
      if (emp.applyPension) {
        if (!emp.pensionTotalSalary || emp.pensionTotalSalary === 0) {
          return false;
        }
      }

      // 건강보험
      if (emp.applyHealthInsurance) {
        if (!emp.healthTotalSalary || emp.healthTotalSalary === 0) {
          return false;
        }
      }

      // 고용보험
      if (emp.applyEmploymentInsurance) {
        if (!emp.employmentTotalSalary || emp.employmentTotalSalary === 0) {
          return false;
        }
      }

      // 산재보험
      if (emp.applyWorkersCompensation) {
        if (!emp.workersCompTotalSalary || emp.workersCompTotalSalary === 0) {
          return false;
        }
      }
    }

    return true;
  }, [workplace, employees]);

  // 신고하기 버튼 클릭 (확인 다이얼로그 열기)
  const handleSubmit = (): void => {
    setConfirmDialogOpen(true);
  };

  // 신고 확인 후 실제 신고 처리
  const handleConfirmSubmit = (): void => {
    // 신고내역 저장
    const reportDate = format(new Date(), 'yyyy-MM-dd');
    saveTotalSalaryHistory(reportDate, workplace, employees);

    // 사업장 정보 저장
    saveWorkplaceInfo(workplace);

    // 임시 저장 데이터 삭제
    localStorage.removeItem(TOTAL_SALARY_TEMP_STORAGE_KEY);
    setHasTempData(false);
    setTempSavedAt('');

    toast({
      title: '보수총액 신고가 완료되었습니다.',
    });

    // 초기화 (기본 직원 1명 추가)
    setEmployees([createDefaultEmployee()]);
    setSelectedEmployeeIds(new Set());
    setConfirmDialogOpen(false);
  };

  // 신고내역 조회 다이얼로그 열기
  const handleOpenHistory = (): void => {
    const end = new Date();
    const start = new Date();
    start.setMonth(start.getMonth() - 1);

    setHistoryStartDate(format(start, 'yyyy-MM-dd'));
    setHistoryEndDate(format(end, 'yyyy-MM-dd'));
    setSelectedPeriod('1month');
    setHistorySearch('');
    setCurrentPage(1);

    // 신고내역 불러오기
    const allHistories = getTotalSalaryHistories();
    setHistories(allHistories);
    setHistoryDialogOpen(true);
  };

  // 기간 버튼 클릭
  const handlePeriodClick = (period: string): void => {
    setSelectedPeriod(period);
    const end = new Date();
    const start = new Date();

    switch (period) {
      case 'today':
        break;
      case '1week':
        start.setDate(start.getDate() - 7);
        break;
      case '1month':
        start.setMonth(start.getMonth() - 1);
        break;
      case '3months':
        start.setMonth(start.getMonth() - 3);
        break;
      case '6months':
        start.setMonth(start.getMonth() - 6);
        break;
      case '1year':
        start.setFullYear(start.getFullYear() - 1);
        break;
    }

    setHistoryStartDate(format(start, 'yyyy-MM-dd'));
    setHistoryEndDate(format(end, 'yyyy-MM-dd'));
  };

  // 신고내역 필터링
  const filteredHistories = useMemo(() => {
    return histories.filter((history) => {
      // 날짜 필터
      const reportDate = new Date(history.reportDate);
      const start = new Date(historyStartDate);
      const end = new Date(historyEndDate);
      end.setHours(23, 59, 59, 999);

      if (reportDate < start || reportDate > end) {
        return false;
      }

      // 검색어 필터
      if (historySearch) {
        const searchLower = historySearch.toLowerCase();
        return history.employees.some(
          (emp: EmployeeTotalSalaryInfo) =>
            emp.name?.toLowerCase().includes(searchLower) ||
            emp.employeeNumber?.toLowerCase().includes(searchLower)
        );
      }

      return true;
    });
  }, [histories, historyStartDate, historyEndDate, historySearch]);

  return (
    <div className="space-y-6 pb-24">
      <PageHeader
        title="보수총액신고"
        showBackButton={false}
        actions={
          <Button variant="outline" onClick={handleOpenHistory}>
            신고내역 조회
          </Button>
        }
      />

      {/* 사업장 정보 */}
      <Card>
        <CardHeader>
          <CardTitle>사업장 정보</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-4 gap-4">
            <div className="space-y-2">
              <Label>사업장관리번호 *</Label>
              <Input
                value={workplace.managementNumber}
                onChange={(e) => handleWorkplaceChange('managementNumber', e.target.value)}
                placeholder="예: 1234567890"
              />
            </div>
            <div className="space-y-2">
              <Label>명칭 *</Label>
              <Input
                value={workplace.name}
                onChange={(e) => handleWorkplaceChange('name', e.target.value)}
                onCompositionStart={() => setIsComposing(true)}
                onCompositionEnd={() => setIsComposing(false)}
                placeholder="사업장명"
              />
            </div>
            <div className="space-y-2">
              <Label>전화번호 *</Label>
              <Input
                value={workplace.phoneNumber}
                onChange={(e) => {
                  const value = e.target.value;
                  if (/^[0-9+()-]*$/.test(value)) {
                    handleWorkplaceChange('phoneNumber', value);
                  }
                }}
                placeholder="예: 02-1234-5678"
              />
            </div>
            <div className="space-y-2">
              <Label>FAX번호</Label>
              <Input
                value={workplace.faxNumber}
                onChange={(e) => {
                  const value = e.target.value;
                  if (/^[0-9-]*$/.test(value)) {
                    handleWorkplaceChange('faxNumber', value);
                  }
                }}
                placeholder="예: 02-1234-5678"
              />
            </div>
          </div>

          <div className="grid grid-cols-4 gap-4">
            <div className="space-y-2">
              <Label>우편번호 *</Label>
              <div className="flex gap-2">
                <Input value={workplace.postalCode} readOnly placeholder="우편번호" />
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => setAddressDialogOpen(true)}
                >
                  <Search className="h-4 w-4 mr-1" />
                  주소 검색
                </Button>
              </div>
            </div>
            <div className="col-span-2 space-y-2">
              <Label>주소 *</Label>
              <Input value={workplace.address} readOnly placeholder="주소" />
            </div>
            <div className="space-y-2">
              <Label>상세주소 *</Label>
              <Input
                value={workplace.addressDetail}
                onChange={(e) => handleWorkplaceChange('addressDetail', e.target.value)}
                onCompositionStart={() => setIsComposing(true)}
                onCompositionEnd={() => setIsComposing(false)}
                placeholder="상세주소"
              />
            </div>
          </div>
        </CardContent>
      </Card>

      {/* 직원 목록 */}
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle>직장가입자 목록</CardTitle>
            <div className="flex gap-2">
              <Button size="sm" onClick={() => setEmployees([...employees, createDefaultEmployee()])}>
                <Plus className="h-4 w-4 mr-1" />
                추가하기
              </Button>
              {selectedEmployeeIds.size > 0 && (
                <Button variant="destructive" size="sm" onClick={handleDeleteEmployees}>
                  <Trash2 className="h-4 w-4 mr-1" />
                  선택 삭제 ({selectedEmployeeIds.size})
                </Button>
              )}
            </div>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          {employees.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              직원을 추가하세요.
            </div>
          ) : (
            <div className="space-y-6">
              {employees.map((employee, index) => (
                <Card key={index} className="border-2">
                  <CardHeader className="bg-gray-50">
                    <div className="flex items-center justify-between">
                      <div className="font-semibold">
                        직원{index + 1}
                      </div>
                      {index > 0 && (
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={() => handleDeleteSingleEmployee(index)}
                          className="h-8 w-8"
                        >
                          <Trash2 className="h-4 w-4" />
                        </Button>
                      )}
                    </div>
                  </CardHeader>
                  <CardContent className="pt-6 space-y-6">
                    {/* 기본 정보 */}
                    <div>
                      <h4 className="font-medium mb-3">기본 정보</h4>
                      <div className="grid grid-cols-4 gap-4">
                        <div className="space-y-2">
                          <Label>성명 *</Label>
                          <EmployeeCombobox
                            value={employee.employeeId}
                            onChange={(selectedEmployee) => handleSelectEmployeeFromCombobox(index, selectedEmployee)}
                            excludeIds={employees
                              .filter((e, i) => i !== index && e.employeeId)
                              .map((e) => e.employeeId as string)}
                          />
                        </div>
                        <div className="space-y-2">
                          <Label>주민등록번호/외국인등록번호 *</Label>
                          <Input
                            value={employee.residentNumber}
                            onChange={(e) => handleResidentNumberChange(index, e.target.value)}
                            placeholder="예: 900101-1234567"
                          />
                        </div>
                        <div className="space-y-2">
                          <Label>신고년도 *</Label>
                          <Input
                            type="text"
                            value={employee.reportYear}
                            onChange={(e) => {
                              const value = e.target.value;
                              if (value === '' || /^\d{0,4}$/.test(value)) {
                                handleEmployeeChange(index, 'reportYear', value);
                              }
                            }}
                            placeholder="예: 2024"
                            maxLength={4}
                          />
                        </div>
                        <div className="space-y-2">
                          <Label>자격취득일</Label>
                          <Input
                            type="date"
                            value={employee.acquisitionDate || ''}
                            onChange={(e) =>
                              handleEmployeeChange(index, 'acquisitionDate', e.target.value)
                            }
                          />
                        </div>
                      </div>
                    </div>

                    {/* 보험 신고 여부 */}
                    <div>
                      <h4 className="font-medium mb-3">보험 신고</h4>
                      <div className="flex gap-6">
                        <div className="flex items-center gap-2">
                          <Checkbox
                            checked={employee.applyPension}
                            onChange={(e) =>
                              handleEmployeeChange(index, 'applyPension', e.target.checked)
                            }
                          />
                          <Label>국민연금</Label>
                        </div>
                        <div className="flex items-center gap-2">
                          <Checkbox
                            checked={employee.applyHealthInsurance}
                            onChange={(e) =>
                              handleEmployeeChange(
                                index,
                                'applyHealthInsurance',
                                e.target.checked
                              )
                            }
                          />
                          <Label>건강보험</Label>
                        </div>
                        <div className="flex items-center gap-2">
                          <Checkbox
                            checked={employee.applyEmploymentInsurance}
                            onChange={(e) =>
                              handleEmployeeChange(
                                index,
                                'applyEmploymentInsurance',
                                e.target.checked
                              )
                            }
                          />
                          <Label>고용보험</Label>
                        </div>
                        <div className="flex items-center gap-2">
                          <Checkbox
                            checked={employee.applyWorkersCompensation}
                            onChange={(e) =>
                              handleEmployeeChange(
                                index,
                                'applyWorkersCompensation',
                                e.target.checked
                              )
                            }
                          />
                          <Label>산재보험</Label>
                        </div>
                      </div>
                    </div>

                    {/* 국민연금 */}
                    {employee.applyPension && (
                      <div className="border-t pt-4">
                        <h4 className="font-medium mb-3">국민연금</h4>
                        <div className="space-y-4">
                          {/* 첫 번째 행: 신고유형, 근무시작일, 보수총액 */}
                          <div className="grid grid-cols-4 gap-4">
                            <div className="space-y-2">
                              <Label>신고유형</Label>
                              <Select
                                value={employee.pensionReportType || ''}
                                onValueChange={(value) =>
                                  handleEmployeeChange(index, 'pensionReportType', value)
                                }
                              >
                                <SelectTrigger>
                                  <SelectValue placeholder="선택" />
                                </SelectTrigger>
                                <SelectContent>
                                  <SelectItem value="정기신고">정기신고</SelectItem>
                                  <SelectItem value="수정신고">수정신고</SelectItem>
                                  <SelectItem value="소급신고">소급신고</SelectItem>
                                </SelectContent>
                              </Select>
                            </div>
                            <div className="space-y-2">
                              <Label>{employee.reportYear || 'YYYY'}년 근무시작일</Label>
                              <Input
                                type="text"
                                value={employee.pensionWorkStartDate || ''}
                                onChange={(e) => {
                                  const value = e.target.value;
                                  // 숫자와 하이픈만 허용
                                  const numbers = value.replace(/[^0-9]/g, '');

                                  // 4자리 제한 (MMDD)
                                  const limited = numbers.slice(0, 4);

                                  // 2자리 이후 하이픈 자동 추가 (MM-DD)
                                  let formatted = limited;
                                  if (limited.length > 2) {
                                    formatted = `${limited.slice(0, 2)}-${limited.slice(2)}`;
                                  }

                                  handleEmployeeChange(index, 'pensionWorkStartDate', formatted);
                                }}
                                onBlur={(e) => handlePensionWorkStartDateBlur(index, e.target.value)}
                                placeholder="MM-DD"
                                maxLength={5}
                                className={pensionWorkStartDateErrors[index] ? 'border-red-500' : ''}
                              />
                              {pensionWorkStartDateErrors[index] && (
                                <p className="text-sm text-red-500">{pensionWorkStartDateErrors[index]}</p>
                              )}
                            </div>
                            <div className="space-y-2">
                              <Label>{employee.reportYear || 'YYYY'}년 보수총액 *</Label>
                              <Input
                                type="text"
                                value={employee.pensionTotalSalary ? employee.pensionTotalSalary.toLocaleString('ko-KR') : ''}
                                onChange={(e) => {
                                  const numericValue = e.target.value.replace(/,/g, '');
                                  if (numericValue === '' || /^\d+$/.test(numericValue)) {
                                    handleEmployeeChange(
                                      index,
                                      'pensionTotalSalary',
                                      numericValue === '' ? 0 : Number(numericValue)
                                    );
                                  }
                                }}
                                placeholder="0"
                              />
                            </div>
                          </div>

                          {/* 두 번째 행: 휴직 체크박스 및 휴직일수 */}
                          <div className="grid grid-cols-4 gap-4">
                            <div className="flex items-center gap-2">
                              <Checkbox
                                checked={employee.pensionHasLeave || false}
                                onChange={(e) =>
                                  handleEmployeeChange(index, 'pensionHasLeave', e.target.checked)
                                }
                              />
                              <Label className="cursor-pointer">신고년도 기간 중 휴직하셨나요?</Label>
                            </div>
                            {employee.pensionHasLeave && (
                              <div className="space-y-2">
                                <Label>실제 휴직일수</Label>
                                <Input
                                  type="text"
                                  value={employee.pensionLeaveDays || ''}
                                  onChange={(e) => {
                                    const value = e.target.value;
                                    // 숫자만 허용
                                    if (value === '' || /^\d+$/.test(value)) {
                                      handleEmployeeChange(
                                        index,
                                        'pensionLeaveDays',
                                        value === '' ? 0 : Number(value)
                                      );
                                    }
                                  }}
                                  onBlur={(e) => {
                                    const value = e.target.value === '' ? 0 : Number(e.target.value);
                                    handlePensionLeaveDaysBlur(index, value);
                                  }}
                                  placeholder="일수 입력"
                                  className={pensionLeaveDaysErrors[index] ? 'border-red-500' : ''}
                                />
                                {pensionLeaveDaysErrors[index] && (
                                  <p className="text-sm text-red-500">{pensionLeaveDaysErrors[index]}</p>
                                )}
                              </div>
                            )}
                          </div>
                        </div>
                      </div>
                    )}

                    {/* 건강보험 */}
                    {employee.applyHealthInsurance && (
                      <div className="border-t pt-4">
                        <h4 className="font-medium mb-3">건강보험</h4>
                        <div className="grid grid-cols-4 gap-4">
                          <div className="space-y-2">
                            <Label>{employee.reportYear || 'YYYY'}년 보수총액 *</Label>
                            <Input
                              type="text"
                              value={employee.healthTotalSalary ? employee.healthTotalSalary.toLocaleString('ko-KR') : ''}
                              onChange={(e) => {
                                const numericValue = e.target.value.replace(/,/g, '');
                                if (numericValue === '' || /^\d+$/.test(numericValue)) {
                                  handleEmployeeChange(
                                    index,
                                    'healthTotalSalary',
                                    numericValue === '' ? 0 : Number(numericValue)
                                  );
                                }
                              }}
                              placeholder="0"
                            />
                          </div>
                          <div className="space-y-2">
                            <Label>근무개월수</Label>
                            <Input
                              type="number"
                              value={employee.healthWorkMonths === 0 ? '' : employee.healthWorkMonths}
                              onChange={(e) => {
                                const value = e.target.value;
                                if (value === '') {
                                  handleEmployeeChange(index, 'healthWorkMonths', 0);
                                  return;
                                }
                                const numValue = Number(value);
                                if (numValue >= 1 && numValue <= 12) {
                                  handleEmployeeChange(index, 'healthWorkMonths', numValue);
                                }
                              }}
                              placeholder="0"
                              min="1"
                              max="12"
                            />
                          </div>
                          <div className="space-y-2">
                            <Label>{employee.reportYear || 'YYYY'}년 보험료 부과 총액</Label>
                            <Input
                              type="text"
                              value={employee.healthInsurancePremiumTotal ? employee.healthInsurancePremiumTotal.toLocaleString('ko-KR') : ''}
                              onChange={(e) => {
                                const numericValue = e.target.value.replace(/,/g, '');
                                if (numericValue === '' || /^\d+$/.test(numericValue)) {
                                  handleEmployeeChange(
                                    index,
                                    'healthInsurancePremiumTotal',
                                    numericValue === '' ? 0 : Number(numericValue)
                                  );
                                }
                              }}
                              placeholder="0"
                            />
                          </div>
                        </div>
                      </div>
                    )}

                    {/* 고용보험 */}
                    {employee.applyEmploymentInsurance && (
                      <div className="border-t pt-4">
                        <h4 className="font-medium mb-3">고용보험</h4>
                        <div className="grid grid-cols-4 gap-4">
                          <div className="space-y-2">
                            <Label>{employee.reportYear || 'YYYY'}년 보수총액 *</Label>
                            <Input
                              type="text"
                              value={employee.employmentTotalSalary ? employee.employmentTotalSalary.toLocaleString('ko-KR') : ''}
                              onChange={(e) => {
                                const numericValue = e.target.value.replace(/,/g, '');
                                if (numericValue === '' || /^\d+$/.test(numericValue)) {
                                  handleEmployeeChange(
                                    index,
                                    'employmentTotalSalary',
                                    numericValue === '' ? 0 : Number(numericValue)
                                  );
                                }
                              }}
                              placeholder="0"
                            />
                          </div>
                        </div>
                      </div>
                    )}

                    {/* 산재보험 */}
                    {employee.applyWorkersCompensation && (
                      <div className="border-t pt-4">
                        <h4 className="font-medium mb-3">산재보험</h4>
                        <div className="grid grid-cols-4 gap-4">
                          <div className="space-y-2">
                            <Label>{employee.reportYear || 'YYYY'}년 보수총액 *</Label>
                            <Input
                              type="text"
                              value={employee.workersCompTotalSalary ? employee.workersCompTotalSalary.toLocaleString('ko-KR') : ''}
                              onChange={(e) => {
                                const numericValue = e.target.value.replace(/,/g, '');
                                if (numericValue === '' || /^\d+$/.test(numericValue)) {
                                  handleEmployeeChange(
                                    index,
                                    'workersCompTotalSalary',
                                    numericValue === '' ? 0 : Number(numericValue)
                                  );
                                }
                              }}
                              placeholder="0"
                            />
                          </div>
                        </div>
                      </div>
                    )}
                  </CardContent>
                </Card>
              ))}
            </div>
          )}
        </CardContent>
      </Card>

      {/* 주소 검색 다이얼로그 */}
      <AddressSearchDialog
        open={addressDialogOpen}
        onOpenChange={setAddressDialogOpen}
        onComplete={handleAddressComplete}
      />

      {/* 신고 확인 다이얼로그 */}
      <Dialog open={confirmDialogOpen} onOpenChange={setConfirmDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>보수총액 신고 확인</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            {/* 직장가입자 목록 */}
            <div>
              <p className="text-sm font-medium mb-2">신고 대상 ({employees.length}명)</p>
              <div className="bg-gray-50 p-3 rounded-md max-h-40 overflow-y-auto">
                {employees.map((emp, index) => (
                  <div key={index} className="text-sm text-gray-700">
                    {emp.name}({emp.employeeNumber || '미입력'})
                  </div>
                ))}
              </div>
            </div>

            {/* 사업장 정보 */}
            <div className="space-y-3">
              <div className="text-sm">
                <span className="text-gray-600">사업장: </span>
                <span className="font-medium">
                  {(() => {
                    const sharedWorkplace = loadWorkplaceInfo();
                    return sharedWorkplace?.address
                      ? getShortAddress(sharedWorkplace.address)
                      : workplace.name || '-';
                  })()}
                </span>
              </div>

              {/* 공단 FAX 번호 */}
              <div className="space-y-2">
                <div className="flex items-center gap-2">
                  <Label htmlFor="faxNumber" className="text-sm font-medium">
                    공단 FAX 번호
                  </Label>
                  <Button
                    type="button"
                    variant="outline"
                    size="sm"
                    onClick={() => window.open('https://www.4insure.or.kr/pbiz/ntcn/selectInstBrofSrchView.do', '_blank')}
                  >
                    FAX번호 찾기
                  </Button>
                </div>
                <Input
                  id="faxNumber"
                  value={faxNumber}
                  onChange={(e) => {
                    const value = e.target.value;
                    // 숫자, -, (, ) 만 허용
                    if (/^[0-9()-]*$/.test(value)) {
                      setFaxNumber(value);
                    }
                  }}
                  placeholder="예: 02-1234-5678"
                  className="text-sm"
                />
                <p className="text-xs text-gray-600">
                  사업장 주소에 맞춰 공단 FAX 번호를 입력해주세요. 1개의 공단 FAX 번호만 입력해주시면 됩니다. (신고하려는 보험에 속하는 공단이어야 함)
                </p>
              </div>
            </div>
          </div>

          {/* 버튼 */}
          <div className="flex justify-end gap-2 mt-4">
            <Button variant="outline" onClick={() => setConfirmDialogOpen(false)}>
              취소
            </Button>
            <Button variant="default" onClick={handleConfirmSubmit}>
              확인
            </Button>
          </div>
        </DialogContent>
      </Dialog>

      {/* 신고내역 조회 다이얼로그 */}
      <Dialog open={historyDialogOpen} onOpenChange={setHistoryDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>신고내역 조회</DialogTitle>
          </DialogHeader>

          <div className="space-y-4">
            <div className="flex gap-2">
              <Input
                type="date"
                value={historyStartDate}
                onChange={(e) => {
                  setHistoryStartDate(e.target.value);
                  setSelectedPeriod('');
                }}
                max="2100-12-31"
                className="w-40"
              />
              <Input
                type="date"
                value={historyEndDate}
                onChange={(e) => {
                  setHistoryEndDate(e.target.value);
                  setSelectedPeriod('');
                }}
                max="2100-12-31"
                className="w-40"
              />
              <Button
                variant={selectedPeriod === 'today' ? 'default' : 'outline'}
                onClick={() => handlePeriodClick('today')}
                size="sm"
              >
                당일
              </Button>
              <Button
                variant={selectedPeriod === '1week' ? 'default' : 'outline'}
                onClick={() => handlePeriodClick('1week')}
                size="sm"
              >
                일주일
              </Button>
              <Button
                variant={selectedPeriod === '1month' ? 'default' : 'outline'}
                onClick={() => handlePeriodClick('1month')}
                size="sm"
              >
                1개월
              </Button>
              <Button
                variant={selectedPeriod === '3months' ? 'default' : 'outline'}
                onClick={() => handlePeriodClick('3months')}
                size="sm"
              >
                3개월
              </Button>
              <Button
                variant={selectedPeriod === '6months' ? 'default' : 'outline'}
                onClick={() => handlePeriodClick('6months')}
                size="sm"
              >
                6개월
              </Button>
              <Button
                variant={selectedPeriod === '1year' ? 'default' : 'outline'}
                onClick={() => handlePeriodClick('1year')}
                size="sm"
              >
                1년
              </Button>
            </div>

            <div className="flex gap-2">
              <Input
                value={historySearch}
                onChange={(e) => setHistorySearch(e.target.value)}
                onCompositionStart={() => setIsComposing(true)}
                onCompositionEnd={() => setIsComposing(false)}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && !isComposing) {
                    setCurrentPage(1);
                  }
                }}
                placeholder="사번 또는 성명"
                className="flex-1"
              />
              <Button
                onClick={() => setCurrentPage(1)}
                onMouseDown={(e) => e.preventDefault()}
              >
                검색
              </Button>
            </div>

            {/* 신고내역 리스트 */}
            <div className="space-y-2">

              {filteredHistories.length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  신고내역이 없습니다.
                </div>
              ) : (
                <>
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead>신고일자</TableHead>
                        <TableHead>사업장명</TableHead>
                        <TableHead>직원</TableHead>
                        <TableHead className="text-center">미리보기</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {filteredHistories
                        .slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage)
                        .map((history, index) => (
                          <TableRow key={index}>
                            <TableCell>{history.reportDate}</TableCell>
                            <TableCell>{history.workplace.name}</TableCell>
                            <TableCell>
                              {history.employees.length}명(
                              {history.employees.map((emp: EmployeeTotalSalaryInfo) => emp.name).join(', ')})
                            </TableCell>
                            <TableCell className="text-center">
                              <Button variant="outline" size="sm">
                                미리보기
                              </Button>
                            </TableCell>
                          </TableRow>
                        ))}
                    </TableBody>
                  </Table>

                  {/* 페이지네이션 */}
                  <div className="flex justify-center gap-2 mt-4">
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => setCurrentPage((prev) => Math.max(1, prev - 1))}
                      disabled={currentPage === 1}
                    >
                      이전
                    </Button>
                    {(() => {
                      const totalPages = Math.ceil(filteredHistories.length / itemsPerPage);
                      const pageNumbers: (number | string)[] = [];

                      if (totalPages <= 7) {
                        for (let i = 1; i <= totalPages; i++) {
                          pageNumbers.push(i);
                        }
                      } else {
                        // 첫 페이지는 항상 표시
                        pageNumbers.push(1);

                        // 현재 페이지가 4 이상이면 ... 표시
                        if (currentPage > 3) {
                          pageNumbers.push('start-ellipsis');
                        }

                        // 현재 페이지 주변 표시 (앞뒤 1페이지)
                        const start = Math.max(2, currentPage - 1);
                        const end = Math.min(totalPages - 1, currentPage + 1);

                        for (let i = start; i <= end; i++) {
                          pageNumbers.push(i);
                        }

                        // 현재 페이지가 끝에서 3페이지 이전이면 ... 표시
                        if (currentPage < totalPages - 2) {
                          pageNumbers.push('end-ellipsis');
                        }

                        // 마지막 페이지는 항상 표시
                        pageNumbers.push(totalPages);
                      }

                      return pageNumbers.map((page) => {
                        if (typeof page === 'string') {
                          // ... 표시
                          return (
                            <span key={page} className="px-2 py-1 text-sm text-gray-400">
                              ...
                            </span>
                          );
                        }

                        return (
                          <Button
                            key={page}
                            variant={currentPage === page ? 'default' : 'outline'}
                            size="sm"
                            onClick={() => setCurrentPage(page)}
                          >
                            {page}
                          </Button>
                        );
                      });
                    })()}
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() =>
                        setCurrentPage((prev) =>
                          Math.min(Math.ceil(filteredHistories.length / itemsPerPage), prev + 1)
                        )
                      }
                      disabled={currentPage === Math.ceil(filteredHistories.length / itemsPerPage)}
                    >
                      다음
                    </Button>
                  </div>
                </>
              )}
            </div>
          </div>
        </DialogContent>
      </Dialog>

      {/* 하단 버튼 영역 */}
      <div className="fixed bottom-0 left-[305px] right-0 z-10">
        <div className="mx-auto max-w-[1500px] px-6">
          <div className={`bg-white pt-4 pb-6 px-6 flex justify-between items-center ${!isAtBottom ? 'border-t shadow-[0_-1px_3px_0_rgba(0,0,0,0.1)]' : ''}`}>
            <div className="flex gap-2">
              {hasTempData && (
                <>
                  <Button variant="outline" onClick={handleLoadTempData}>
                    임시저장 불러오기
                  </Button>
                  <Button variant="outline" onClick={handleClearTempData}>
                    임시저장 삭제
                  </Button>
                  {tempSavedAt && (
                    <span className="text-sm text-gray-500 flex items-center">
                      마지막 저장: {format(new Date(tempSavedAt), 'yyyy-MM-dd HH:mm:ss')}
                    </span>
                  )}
                </>
              )}
            </div>
            <div className="flex gap-2">
              <Button variant="outline" onClick={handleTempSave}>
                임시 저장
              </Button>
              <Button variant="outline">미리보기</Button>
              <Button variant="default" onClick={handleSubmit} disabled={!isFormValid}>
                신고하기
              </Button>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
