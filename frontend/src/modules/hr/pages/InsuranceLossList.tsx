...
import { createLossReportColumns } from '../components/loss-report-columns';
import type { LossReport } from '../types/insuranceReport';
import { toast } from '@/shared/hooks/use-toast';

export function InsuranceLossList(): JSX.Element {
  const [allReports, setAllReports] = useState<LossReport[]>([]);
  ...
  const loadReports = async (): Promise<void> => {
    setIsLoading(true);
    try {
      const response = await lossReportService.getAll({ page: 1, limit: 10000 });
  ...
  const handleNewReport = (): void => {
    window.history.pushState({}, '', '/hr/insurance/loss/new');
  ...
  const handleDelete = async (id: string): Promise<void> => {
    ...
    if (!confirmed) return;
    try {
      await lossReportService.delete(id);
  ...
  const handlePreview = (id: string): void => {
    window.open(`/hr/insurance/loss/detail/${id}`, '_blank');
  };

  const handleRowClick = (report: LossReport): void => {
    if (report.status === 'draft') {
      window.history.pushState({}, '', `/hr/insurance/loss/edit/${report.id}`);
      window.dispatchEvent(new PopStateEvent('popstate'));
    } else {
      window.history.pushState({}, '', `/hr/insurance/loss/detail/${report.id}`);
      window.dispatchEvent(new PopStateEvent('popstate'));
    }
  };
  ...
  const columns = createLossReportColumns({
  ...
  return (
    <>
      <div>
        <PageHeader title="자격상실신고" showBackButton={false} />
