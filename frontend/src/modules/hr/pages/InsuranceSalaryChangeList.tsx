...
import { createSalaryChangeReportColumns } from '../components/salary-change-report-columns';
import type { SalaryChangeReport } from '../types/insuranceReport';
import { toast } from '@/shared/hooks/use-toast';

export function InsuranceSalaryChangeList(): JSX.Element {
  const [allReports, setAllReports] = useState<SalaryChangeReport[]>([]);
  ...
  const loadReports = async (): Promise<void> => {
    setIsLoading(true);
    try {
      const response = await salaryChangeReportService.getAll({ page: 1, limit: 10000 });
  ...
  const handleNewReport = (): void => {
    window.history.pushState({}, '', '/hr/insurance/salary-change/new');
  ...
  const handleDelete = async (id: string): Promise<void> => {
    ...
    try {
      await salaryChangeReportService.delete(id);
  ...
  const handlePreview = (id: string): void => {
    window.open(`/hr/insurance/salary-change/detail/${id}`, '_blank');
  };

  const handleRowClick = (report: SalaryChangeReport): void => {
    if (report.status === 'draft') {
      window.history.pushState({}, '', `/hr/insurance/salary-change/edit/${report.id}`);
      window.dispatchEvent(new PopStateEvent('popstate'));
    } else {
      window.history.pushState({}, '', `/hr/insurance/salary-change/detail/${report.id}`);
      window.dispatchEvent(new PopStateEvent('popstate'));
    }
  };
  ...
  const columns = createSalaryChangeReportColumns({
  ...
  return (
    <>
      <div>
        <PageHeader title="보수월액 변경신고" showBackButton={false} />
          ...
          <DialogContent className="max-w-sm">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <a
                  href="https://nps.or.kr/elctcvlcpt/etc/getOHAC0070P0.do"
                  target="_blank"
                ...
                <a
                  href="https://www.comwel.or.kr/comwel/fax_pop3.jsp"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-between w-full px-4 py-3 rounded-md border border-slate-200 hover:bg-gray-50 text-sm font-medium"
                >
