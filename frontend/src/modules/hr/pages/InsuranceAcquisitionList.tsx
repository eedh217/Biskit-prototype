...
import { createAcquisitionReportColumns } from '../components/acquisition-report-columns';
import type { AcquisitionReport } from '../types/insuranceReport';
import { toast } from '@/shared/hooks/use-toast';

export function InsuranceAcquisitionList(): JSX.Element {
  const [allReports, setAllReports] = useState<AcquisitionReport[]>([]);
  ...
  const loadReports = async (): Promise<void> => {
    setIsLoading(true);
    try {
      const response = await acquisitionReportService.getAll({ page: 1, limit: 10000 });
  ...
  const handleNewReport = (): void => {
    window.history.pushState({}, '', '/hr/insurance/acquisition/new');
  ...
  const handleDelete = async (id: string): Promise<void> => {
    ...
    try {
      await acquisitionReportService.delete(id);
      toast({
        title: '삭제 완료되었습니다.',
      });
      loadReports();
  ...
  const handlePreview = (id: string): void => {
    window.open(`/hr/insurance/acquisition/detail/${id}`, '_blank');
  };

  // 행 클릭
  const handleRowClick = (report: AcquisitionReport): void => {
    if (report.status === 'draft') {
      // 작성중 → 신고 화면 (편집)
      window.history.pushState({}, '', `/hr/insurance/acquisition/edit/${report.id}`);
      window.dispatchEvent(new PopStateEvent('popstate'));
    } else {
      // 신고완료 → 상세 화면
      window.history.pushState({}, '', `/hr/insurance/acquisition/detail/${report.id}`);
      window.dispatchEvent(new PopStateEvent('popstate'));
    }
  };
  ...
  const columns = createAcquisitionReportColumns({
  ...
  return (
    <>
      <div>
        <PageHeader title="자격취득신고" showBackButton={false} />
